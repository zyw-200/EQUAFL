...
/* EQUAFL 
    We identify application launch varaibles including arguments and environment variables by instrumenting the "do_execve" function.
    You can refer to the function "execve_analysis_vmi".
...
/*
void execve_analysis_vmi_old(CPUState *env, process* pe)
{
    target_ulong argv_pt =0, env_pt=0;
            
    CPUArchState *env_new = env->env_ptr;
    target_ulong pro_pt = env_new->active_tc.gpr[4];
    target_ulong argv_addr = env_new->active_tc.gpr[5];
    target_ulong env_addr = env_new->active_tc.gpr[6];
    DECAF_read_ptr(env, argv_addr, &argv_pt);
    DECAF_read_ptr(env, env_addr, &env_pt);

    char pro_name[100],test_name[100], id_str[256],output_file[256];
    int program_id;

    DECAF_read_mem(env, pro_pt, 100, pro_name);
    getconfig("program_analysis",test_name);
    getconfig("program_analysis",test_name);
    DECAF_printf("sssssssssssssssssssssname:%s, %s,%s\n", pe->name, pro_name, test_name);
    if(strstr(pro_name,test_name) != 0 && strstr(pro_name,"S") == NULL)
    {
        //DECAF_printf("name:%s, %s,%s\n", pe->name, pro_name, test_name);
        DECAF_printf("name:%s, %x,%x,%x,%x\n", pro_name, argv_addr, env_addr, argv_pt, env_pt);
        getconfig("id", id_str);
        assert(strlen(id_str) > 0);
        program_id = strtol(id_str, NULL, 10);
        sprintf(output_file,"args_file/args_%s", id_str);
        FILE * fp = fopen(output_file, "w+");

        int index = 0;
        while(argv_pt!=0)
        {
            int ret =DECAF_read_mem(env, argv_pt, 100, pe->args[index]);
            DECAF_printf("arg:%x,%s, %d\n", argv_pt,  pe->args[index], ret);

            fprintf(fp, "%s:", pe->args[index]);
            argv_addr+=4;
            DECAF_read_ptr(env, argv_addr, &argv_pt);
            index++;
        } 
        ...
        while(env_pt!=0)
        {
            int ret = DECAF_read_mem(env, env_pt, 100, pe->envs[index]);
            DECAF_printf("env:%x,%s,%d\n", env_pt, pe->envs[index], ret);
            fprintf(fp, "%s:", pe->envs[index]);
            env_addr+=4;
            DECAF_read_ptr(env, env_addr, &env_pt);
            index++;
...
void execve_analysis_vmi(CPUState *env, char * process_name)
{//search_binary_handler before block
...
/*
    ...
    target_ulong pro_pt = env_new->active_tc.gpr[4] + 0xb0; //a0
    target_ulong argv_addr = env_new->active_tc.gpr[17]; //s1
    target_ulong env_addr = env_new->active_tc.gpr[18]; //s2
#else
    target_ulong pro_pt = env_new->active_tc.gpr[4] + 0xb0; //a0
    target_ulong argv_addr = env_new->active_tc.gpr[20]; //s4 
    target_ulong env_addr = env_new->active_tc.gpr[21]; //s5 
    //target_ulong argv_addr = env_new->active_tc.gpr[19]; //s3  args  2.6.39.4+
    //target_ulong env_addr = env_new->active_tc.gpr[20]; //s4  envs2.6.39.4+
#endif
*/
    target_ulong pro_pt = env_new->active_tc.gpr[4] + 0xb0; //a0
    target_ulong argv_addr = env_new->active_tc.gpr[19]; //s3  args  2.6.39.4+
...
#ifdef TARGET_ARM
    target_ulong pro_pt = env_new->regs[4] + 0xb0; //r4
    target_ulong argv_addr = env_new->regs[13];  //sp
    DECAF_read_ptr(env, argv_addr, &argv_addr);
    target_ulong env_addr = env_new->regs[9]; //r9
#endif

    DECAF_read_ptr(env, argv_addr, &argv_pt);
    DECAF_read_ptr(env, env_addr, &env_pt);

    char pro_name[100], type_name[100], id_str[256],output_file[256];
    int program_id;

    target_ulong family_name_pt = 0;
    int rret = DECAF_read_ptr(env, pro_pt, &family_name_pt);
    DECAF_read_mem(env, family_name_pt, 100, pro_name);
    //printf("find procname:%s\n", pro_name);
#endif
    char test_name[100];
    getconfig("program_analysis",test_name);
    getconfig("program_analysis",test_name);
    //printf("xxxxxxxxxxxxxxxxxxxxxxx:%x, %s,%s,%d\n",family_name_pt, pro_name, test_name, rret);
    //10670 sh /var/servd/HTTP_start.sh
#ifdef ARGU_ANALYSIS
    //if(strstr(pro_name,test_name) != 0 && strstr(pro_name,"S") == NULL&& strstr(pro_name,".sh") == NULL)
    if(strstr(pro_name,test_name) != 0 && strstr(pro_name,".sh") == NULL && strstr(pro_name,"init.d") == NULL && strstr(pro_name,"rc.d") == NULL) // 105943 etc/init.d/uhttpd POSIX shell script, ASCII text executable //108893 /etc/rc.d/S66uhttpd
#elif defined(FILE_ANALYSIS)
    if(strstr(process_name,test_name) != 0 && strstr(process_name,"S") == NULL&& strstr(process_name,".sh") == NULL) //no path, so "init.d" not required
#endif
    {
#ifdef ARGU_ANALYSIS
        DECAF_printf("name:%s, %x,%x,%x,%x\n", pro_name, argv_addr, env_addr, argv_pt, env_pt);
        ...
        assert(strlen(id_str) > 0);
        program_id = strtol(id_str, NULL, 10);
        getconfig("program_type", type_name);
        assert(strlen(type_name) > 0);
        sprintf(output_file,"args_file/%s/%s_%s", type_name, id_str, type_name);
        FILE * fp = fopen(output_file, "w+");

        int index = 0;
        while(argv_pt!=0)
        {
            int ret =DECAF_read_mem(env, argv_pt, 100, args[index]);
            DECAF_printf("arg:%x,%s, %d\n", argv_pt,  args[index], ret);
            //if(ret != -1)
            fprintf(fp, "%s^", args[index]);
            argv_addr+=4;
            DECAF_read_ptr(env, argv_addr, &argv_pt);
            index++;
        } 
        ...
        while(env_pt!=0)
        {
            int ret = DECAF_read_mem(env, env_pt, 100, envs[index]);
            DECAF_printf("env:%x,%s,%d\n", env_pt, envs[index], ret);
            //if(ret != -1)
            fprintf(fp, "%s^", envs[index]);
            env_addr+=4;
            DECAF_read_ptr(env, env_addr, &env_pt);
            index++;
...
#ifdef FILE_ANALYSIS
            execve_analysis_vmi(env, pe->name);
#endif
...
#elif defined(FILE_ANALYSIS)
    if(OFFSET_PROFILE.proc_exec_connector != pc && OFFSET_PROFILE.proc_fork_connector != pc) //zyw
#else
    if(0)
#endif
        return;
